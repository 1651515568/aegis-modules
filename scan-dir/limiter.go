...
//
// 并发信号量只限「同时在飞的请求数」,不限「每秒请求数」。对真实目标高速爆破会触发
// WAF / IP 封禁,且真实存在的路径若被限流网关挡下回 429/503 时若当「不存在」丢弃会漏报。
// 本文件提供:
//   * limiter:进程内最小间隔限速器,所有探测请求(含基线)统一过闸,平滑到 ~N req/s。
...
// 优先采信整数秒形式的 Retry-After 头;否则指数退避。结果夹在 [backoffBase, backoffCap]。纯函数。
...
func isThrottle(code int) bool { return code == 429 || code == 503 }

// trimSpace 去掉前后空白(Retry-After 可能带前后空格)。
